...
export default function StoryTable({ rows }) {
    ...
    const getStatusStyles = (status, count) => {
        switch (status) {
            case "Flagged":
                return { bg: "#FEFCE8", dot: "#EAB308", text: "#9CA3AF", label: "Flagged" };
            case "Reported":
                return { bg: "#FEF2F2", dot: "#EF4444", text: "#9CA3AF", label: `Reported (${count})` };
            case "Active":
                return { bg: "#F0FDF4", dot: "#22C55E", text: "#9CA3AF", label: "Active" };
            case "Expired":
                return { bg: "#F9FAFB", dot: "#6B7280", text: "#9CA3AF", label: "Expired" };
            default:
    ...
    return <CommonTable columns={columns} rows={rows} onRowClick={handleRowClick} />;
}
